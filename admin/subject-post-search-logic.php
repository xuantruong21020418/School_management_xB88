...
if (isset($_GET['topic-search']) && isset($_GET['submit'])) {
  $search_subject_id = filter_var($_GET['subject_id'], FILTER_SANITIZE_NUMBER_INT);
  $search = filter_var($_GET['topic-search'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
  if ($search == NULL) {
      header('location: ' . ROOT_URL . 'admin/single-subject.php?id=' . $search_subject_id);
      die();
  } else {
    ...
    WHERE sj.subject_id = $search_subject_id AND tc.class LIKE '$class_name' AND st.email LIKE '$email' AND tp.title LIKE '%$search%'
        ...
        <main>
            ...
            <table>
              ...
              <?php
                ...
                $total_pages_sql = "SELECT COUNT(tp.teacher_post_id)
                FROM sms_teacher_posts tp
                JOIN sms_user u ON tp.teacher_id = u.id
                JOIN sms_teacher tc ON tc.email = u.email
                JOIN sms_subjects sj ON tc.subject = sj.subject
