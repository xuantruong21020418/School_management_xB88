...
if (isset($_GET['topic-search']) && isset($_GET['submit'])) {
  $search_class_id = filter_var($_GET['class_id'], FILTER_SANITIZE_NUMBER_INT);
  $search = filter_var($_GET['topic-search'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
  if ($search == NULL) {
      header('location: ' . ROOT_URL . 'admin/single-class.php?id=' . $search_class_id);
      die();
  } else {
      ...
      WHERE tp.title LIKE '%$search%' AND tp.class_id = $search_class_id AND tp.teacher_id = $id
    ...
    <div class="container dashboard__noti__container">
        <main>
            <div class="header">
                <h2>Notifications</h2>
                <div class="add-noti">
                    <a href="<?= ROOT_URL ?>admin/add-post.php?id=<?= $search_class_id ?>" class="btn discuss">
